...
export class MessageRepository {
      ...
      if (existingId) {
        ...
        if (member) {
          ...
          if (raw) {
            ...
            return {
              messageId: existingId as string,
        ...
        return { messageId: existingId as string, expiresInSeconds: ttlSeconds, createdAt: Date.now(), expiresAt: Date.now() + ttlSeconds * 1000 };
      }
    }
  ...
  async markDelivered(userId: string, deviceId: string, messageId: string) {
    ...
    try {
      const parsed = JSON.parse(raw as string) as StoredEncryptedMessage;
